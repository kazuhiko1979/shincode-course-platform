...
3. **数値の主張は一次情報で再計算する**：この種の PR は「総進捗 N/M」「成熟度 X/30（Y%）」「テスト数」
   「PR 番号」「日付」を人手で書き換えるため、算術ずれ・git 履歴との食い違いが実際に発生する。
   表の列和・スコアの合計・`git log --format=%ad`・`npm test` の出力と突き合わせて検算する。
