...
3. **数値の主張は一次情報で再計算する**：この種の PR は「総進捗 N/M」「成熟度 X/30（Y%）」「テスト数」
   ...
   日付は**作業日を書く習慣**があり commit 日と 1 日ずれることが多い（「git log で検証済み」と書かれた行でも実際は不一致）。

4. **「比例した検証」「過剰検証の回避」系の新ルールは、無条件ゲートを薄めないかを見る**：
   このリポジトリには無条件の強制（`npm run verify` の実行義務・push は常に `/push-review` の 3 並列・
   認証変更は 3 ロール確認・hooks/settings 変更は人間承認）がある。効率化を謳う新しい散文ルールは、
   これらの「常に必須」を条件付きに読み替えられる余地を作りやすい（表の列配置・括弧書きの例示が典型）。
